...
for(i in 1:length(chrlen)){
    ...
    stc = offsets[i]
    edc = offsets[i+1]
    system(paste0(file.path(builddir,'validation'),' --output=',sumdir,'/fitted.',i,'.bin --genome=',file.path(inputdir,'genome.in'),' --xopt=\'',outdir,'/yall_',bestreg,'.bin\' --start=',stc+1,' --end=',edc,' --x0=',bx0,' --x0nt=',bx0nt,' && touch /tmp/',i,'.done'))
}
